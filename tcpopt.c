...
/* TODO: параметры программы, в данном случае, минимальный MSS.
...
/* TODO: пример: проверка, что MSS не менее заданного.
...
check_option(
        ...
        enum Result *result) {
    /* Подавляем предупреждение компилятора, что не пользуемся size. */
    (void)size;
...
/* Тело фильтра универсальное для анализа любых опций. */
...
filter(Context ctx) {
    ...
    /* TODO: если нас интересуют только опции в SYN и SYN+ACK */
    ...
    for (i = 0; i < MAX_OPTS; i++) {
        ...
        default:
            ...
            if (check_option(
                        params, type, size, options + offset + 2, &result)) {
                return result;
            }
        }
